...
- (BOOL)ch_isAlphanumericCharacter {
    if (!self.length) return NO;
...
- (BOOL)_ch_isAreaCodeFromCardIDOfPRC {
    ...
    return [dict ch_containsObjectForKey:self];
}
...
- (BOOL)ch_isValidCardIDOfPRC {
    ...
    if (![sProvince _ch_isAreaCodeFromCardIDOfPRC]) return NO;
...
- (BOOL)ch_isValidCardIDOfHK {
    ...
    BOOL validate = [self ch_matchesRegex:regex options:0];
    ...
    if (validate) {
        ...
        if (remainder == 10) {
            validate = [lastCode ch_matchesRegex:@"^[Aa]" options:0];
        } else {
            if ([lastCode ch_matchesRegex:@"^[Aa]" options:0]) {
                validate = NO;
            } else {
