...
@implementation UIImage (CHGIFImage)
...
static NSTimeInterval CHCGImageSourceGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index) {
...
+ (UIImage *)ch_imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    CGImageSourceRef source = CGImageSourceCreateWithData((__bridge CFTypeRef)(data), NULL);
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = CHCGImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
...
+ (UIImage *)ch_imageWithSmallGIFNamed:(NSString *)name {
    CGFloat scale = [UIScreen mainScreen].scale;
    NSString *retinaPath = [[NSBundle mainBundle] pathForResource:name ofType:@"gif"];
    NSData *data = [NSData dataWithContentsOfFile:retinaPath];
    return [self ch_imageWithSmallGIFData:data scale:scale];
}
